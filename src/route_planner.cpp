...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
    ...
    // filling the H value of start_node
    this->start_node->h_value = this->CalculateHValue(end_node);
}
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
    ...
    // looping neighbors to fill the neighbor's attributes
...
// Compare the values of open list to sort it
bool compare(const RouteModel::Node *firstNode, RouteModel::Node *secondNode){
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    // get the lowets node
    RouteModel::Node *pointerToLowestNode = this->open_list.back();
    
    // remove the lowest node
    this->open_list.pop_back();

    return pointerToLowestNode;
}
...
void RoutePlanner::AStarSearch() {
    ...
    // TODO: Implement your solution here.
    // add the curent node to open list
    // fill all the attributes of current node
